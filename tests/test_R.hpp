...
TEST_CASE("Testing apply function") {
  ...
  /*
  SUBCASE("Apply to depth 0") {

    auto vvFunc = [](const std::vector<std::vector<int>> &v) {
      auto result{v};
      result.push_back({5});
      return result;
    };

    auto result = apply(v, vvFunc);
    CHECK(result == std::vector<std::vector<int>>{{1, 2}, {3, 4}, {5}});
    }*/

  SUBCASE("Apply to depth 1") {

    auto vFunc = [](const std::vector<int> &v) {
      auto result{v};
      result.push_back(5);
      return result;
    };

    auto result = apply(v, vFunc, 1);
    CHECK(result == std::vector<std::vector<int>>{{1, 2, 5}, {3, 4, 5}});
  }

  SUBCASE("Apply with depth 2") {
    auto func = [](int x) { return 2 * x; };
    auto result = apply(v, func, 2);
    CHECK(result == std::vector<std::vector<int>>{{2, 4}, {6, 8}});
  }

  SUBCASE("Apply with depth -1") {
    auto func = [](int x) { return 2 * x; };
    auto result = apply(v, func, -1);
    CHECK(result == std::vector<std::vector<int>>{{2, 4}, {6, 8}});
  }
}
